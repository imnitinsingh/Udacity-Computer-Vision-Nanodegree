...
class robot:
    ...
    def sense(self):
        ...
        measurements = [] # create placeholder list
        
        for idx, landmark in enumerate(self.landmarks):
            # compute dx, dy and add raqndom measurement noise
            dx = landmark[0] - self.x + self.rand()*self.measurement_noise
            dy = landmark[1] - self.y + self.rand()*self.measurement_noise
            
            rad = self.measurement_range
            
            # logic to check if the measurement falls within the range
            if rad == -1 or (dx > -rad and dx < rad and dy > -rad and dy < rad):
